...
Page({
  ...
  onLoad: function(options) {
    ...
    if (!drinkId) {
      wx.showToast({ title: '饮品ID缺失', icon: 'none' });
      wx.navigateBack();
      return;
    }
    
    this.setData({ drinkId });
  ...
  checkExistingInCart() {
    ...
    if (existingItem) {
      // 商品已存在，使用购物车中的实际数量
      this.setData({ quantity: existingItem.quantity });
    } else {
      // 商品不存在，重置为默认数量
  ...
  getMockDrinkData: function() {
    return {
      '01': {
        ...
        themeOptions: [
          { value: 'xl', text: '限量方形杯', price: 0 },
          { value: 'cg', text: '常规杯', price: 0 },
        ],
        sizeOptions: [
          { value: '500ml', text: '中杯500ml', price: 0 },
        ],
        tasteOptions:[
          { value: 'mr', text: '默认', price: 0 },
          { value: 'ysj', text: '加益生菌', price: 0 },
        ],
        tempOptions: [
          { value: 'ice', text: '冰' },
          { value: 'no_ice', text: '去冰' }
        ],
        sweetOptions:[
          { value: 'blwjt', text: '不另外加糖' }
        ],
      },
      '02': {
        id: '02',
        ...
        sizeOptions: [
          { value: '500ml', text: '中杯500ml', price: 0 },
          { value: '650ml', text: '大杯650ml', price: 3 }
        ],
        tempOptions: [
          { value: 'ice', text: '冰沙' },
          { value: 'no_ice', text: '去冰' },
        ...
        teaOptions: [
          { value: 'green', text: '绿妍茶底' },
          { value: 'black', text: '红茶茶底' }
        ],
        toppingOptions: [
          { value: 'jelly', text: '加椰果', price: 2 },
          { value: 'bubble', text: '加珍珠', price: 3 }
        ]
      },
      '03': {
        id: '03',
        ...
        sizeOptions: [
          { value: '500ml', text: '中杯500ml', price: 0 },
          { value: '650ml', text: '大杯650ml', price: 5 }
        ],
        tempOptions: [
          { value: 'ice', text: '冰沙' },
          { value: 'no_ice', text: '去冰' }
        ...
        teaOptions: [
          { value: 'green', text: '绿妍茶底' },
          { value: 'jasmine', text: '茉莉茶底' }
        ],
        toppingOptions: [
          { value: 'mango', text: '加芒果果肉', price: 5 },
          { value: 'pudding', text: '加布丁', price: 3 }
        ]
      },
      '04': {
        id: '04',
...
decreaseQuantity() {
  ...
  if (cartItems[index].quantity > this.data.minQuantity) {
    ...
    this.setData({ quantity: cartItems[index].quantity });
  } else {
    // 数量为1时，移除商品
    this.removeFromCart();
  }
...
increaseQuantity() {
  ...
  if (index === -1) {
    ...
    wx.showToast({ title: `最多只能选择${this.data.maxQuantity}件`, icon: 'none' });
  }
},
