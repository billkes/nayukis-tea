// pages/secondPages/trolley/trolley.js
...
Page({
  ...
  handleGetList() {
    const openid = this.data.orderData.openid
    const page = 1
    const pageSize = 10
    const token = this.data.token
    getUserOrders(openid, page, pageSize, token).then(res => {
  ...
  handlePay() {
    ...
    // 调用支付流程
    processPayment(orderData, this.data.token)
      .then(result => {
   ...
   * 生命周期函数--监听页面加载
   */
  onLoad(options) { // 获取用户token和openid
